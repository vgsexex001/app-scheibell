...
class _ClinicActivitiesScreenState extends State<ClinicActivitiesScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      context.read<ClinicContentProvider>().loadContentsByType('ACTIVITIES');
    });
  }
                    ...
                    : filteredPatients.isEmpty
                        ...
                        : ListView.separated(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return _PatientCard(patient: patient, onTap: () => _navigateToPatientDetail(patient));
                            },
                          ),
              ),
            ],
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: const Icon(Icons.directions_run_outlined, color: Color(0xFF697282), size: 32),
          ),
          const SizedBox(height: 20),
          const Text(
            'Selecione um paciente',
            style: TextStyle(color: Color(0xFF212621), fontSize: 16, fontFamily: 'Inter', fontWeight: FontWeight.w600),
          ),
          const SizedBox(height: 8),
          const Text(
            'para visualizar e gerenciar\nas atividades permitidas',
            textAlign: TextAlign.center,
            style: TextStyle(color: Color(0xFF697282), fontSize: 14, fontFamily: 'Inter', fontWeight: FontWeight.w400),
          ),
        ],
      ),
    );
        ...
        child: Row(
          children: [
            Container(
              width: 40,
              height: 40,
              decoration: BoxDecoration(color: const Color(0xFF4F4A34), borderRadius: BorderRadius.circular(20)),
              child: Center(
                child: Text(
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(patient.name, style: const TextStyle(color: Color(0xFF212621), fontSize: 14, fontFamily: 'Inter', fontWeight: FontWeight.w500)),
                  const SizedBox(height: 2),
                  Text((patient.phone?.isNotEmpty ?? false) ? patient.phone! : 'Sem telefone', style: const TextStyle(color: Color(0xFF697282), fontSize: 12, fontFamily: 'Inter')),
                ],
              ),
            ),
            if (patient.dayPostOp != null)
              Container(
                ...
                decoration: BoxDecoration(color: const Color(0xFFA49E86).withAlpha(26), borderRadius: BorderRadius.circular(8)),
                child: Text('D+${patient.dayPostOp}', style: const TextStyle(color: Color(0xFF4F4A34), fontSize: 11, fontFamily: 'Inter', fontWeight: FontWeight.w600)),
              ),
...
class _PatientActivitiesDetailScreenState extends State<_PatientActivitiesDetailScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (provider.currentType != 'ACTIVITIES') {
        provider.loadContentsByType('ACTIVITIES');
      }
    });
  }
              ...
              child: Consumer<ClinicContentProvider>(
                builder: (context, provider, _) {
                  if (provider.isLoadingContents) {
                    return const Center(child: CircularProgressIndicator(color: Color(0xFFA49E86)));
                  }
                  return TabBarView(
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 12),
          const Text('Atividades', style: TextStyle(color: Color(0xFF4F4A34), fontSize: 24, fontFamily: 'Inter', fontWeight: FontWeight.w500)),
        ],
      ),
    );
  }
  ...
  Widget _buildPatientInfo() {
    return Container(
      ...
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        gradient: const LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Color(0xFF4F4A34), Color(0xFF212621)]),
        ...
        children: [
          Container(
            ...
            child: Center(
              child: Text(
                ...
                style: const TextStyle(color: Colors.white, fontSize: 20, fontFamily: 'Inter', fontWeight: FontWeight.w600),
              ),
            ),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(widget.patient.name, style: const TextStyle(color: Colors.white, fontSize: 16, fontFamily: 'Inter', fontWeight: FontWeight.w600)),
                const SizedBox(height: 2),
                Text(
                  widget.patient.dayPostOp != null ? 'D+${widget.patient.dayPostOp} pós-operatório' : (widget.patient.phone ?? 'Sem telefone'),
                  style: TextStyle(color: Colors.white.withAlpha(179), fontSize: 13, fontFamily: 'Inter'),
                ),
              ],
            ),
          ),
  ...
  Widget _buildTabBar() {
    return Container(
      ...
      decoration: const BoxDecoration(
        ...
        border: Border(top: BorderSide(color: Color(0xFFE5E7EB)), left: BorderSide(color: Color(0xFFE5E7EB)), right: BorderSide(color: Color(0xFFE5E7EB))),
      ),
      child: TabBar(
        controller: _tabController,
            ...
            child: AnimatedBuilder(
              ...
              builder: (context, _) {
                ...
                return Container(
                  ...
                  decoration: BoxDecoration(border: Border(bottom: BorderSide(color: isSelected ? cat.color : Colors.transparent, width: 2))),
                  child: Row(
                    ...
                    children: [
                      ...
                      Flexible(
                        child: Text(cat.name, style: TextStyle(color: isSelected ? cat.textColor : const Color(0xFF697282), fontSize: 11, fontFamily: 'Inter', fontWeight: FontWeight.w500), overflow: TextOverflow.ellipsis),
                      ),
                    ],
                  ),
                );
    ...
    return Padding(
      ...
      child: GestureDetector(
        ...
        child: Container(
          width: double.infinity,
          height: 36,
          decoration: BoxDecoration(color: const Color(0xFF4F4A34), borderRadius: BorderRadius.circular(12)),
          child: const Row(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Icon(Icons.add, color: Colors.white, size: 16),
              SizedBox(width: 8),
              Text('Adicionar Atividade', style: TextStyle(color: Colors.white, fontSize: 14, fontFamily: 'Inter', fontWeight: FontWeight.w500)),
            ],
          ),
        ),
      ),
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            const Text('Nenhum item cadastrado', style: TextStyle(color: Color(0xFF697282), fontSize: 16, fontFamily: 'Inter')),
            const SizedBox(height: 8),
            const Text('Toque em "Adicionar Atividade"', style: TextStyle(color: Color(0xFF697282), fontSize: 14, fontFamily: 'Inter')),
          ],
        ),
      );
    }
    ...
    return ReorderableListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return Padding(
          ...
          child: _ContentCard(item: item, category: category, onToggle: () => _toggleItem(item.id), onDelete: () => _deleteItem(item.id), onEdit: () => _showEditModal(item)),
        );
      },
    );
  }
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(success ? 'Item excluído' : 'Erro ao excluir')));
              }
            },
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => _ContentFormModal(
        ...
        onSave: (title, description) async {
          final success = await context.read<ClinicContentProvider>().createContent(type: 'ACTIVITIES', category: category.id, title: title, description: description.isNotEmpty ? description : null);
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(success ? 'Item adicionado' : 'Erro ao adicionar')));
  ...
  void _showEditModal(ClinicContent item) {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => _ContentFormModal(
        ...
        item: item,
        onSave: (title, description) async {
          final success = await context.read<ClinicContentProvider>().updateContent(item.id, title: title, description: description.isNotEmpty ? description : null);
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(success ? 'Item atualizado' : 'Erro ao atualizar')));
          }
        },
      ),
    );
                  ...
                  child: Column(
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(color: const Color(0xFFF5F3EF), borderRadius: BorderRadius.circular(12)),
                        child: Text(item.title, style: const TextStyle(color: Color(0xFF212621), fontSize: 14, fontFamily: 'Inter', fontWeight: FontWeight.w500)),
                      ),
                      if (item.description != null && item.description!.isNotEmpty) ...[
                        ...
                        Container(
                          ...
                          padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                          decoration: BoxDecoration(color: const Color(0xFFF5F3EF), borderRadius: BorderRadius.circular(12)),
                          child: Text(item.description!, style: const TextStyle(color: Color(0xFF212621), fontSize: 14, fontFamily: 'Inter', fontWeight: FontWeight.w400)),
                        ),
                      ],
                    ],
                  ),
              ...
              children: [
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(color: item.isActive ? const Color(0xFF00A63E).withAlpha(26) : const Color(0xFF697282).withAlpha(26), borderRadius: BorderRadius.circular(8)),
                    child: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
...
class _ContentFormModal extends StatefulWidget {
  ...
  const _ContentFormModal({required this.category, this.item, required this.onSave});
...
class _ContentFormModalState extends State<_ContentFormModal> {
  ...
  void _handleSave() {
    ...
    if (title.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Informe o título')));
      return;
    }
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Text(isEditing ? 'Editar Atividade' : 'Nova Atividade', style: const TextStyle(color: Color(0xFF212621), fontSize: 20, fontFamily: 'Inter', fontWeight: FontWeight.w600)),
            ...
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
              decoration: BoxDecoration(color: widget.category.color.withAlpha(26), borderRadius: BorderRadius.circular(8)),
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(widget.category.icon, color: widget.category.color, size: 16),
                  const SizedBox(width: 6),
                  Text(widget.category.name, style: TextStyle(color: widget.category.textColor, fontSize: 12, fontFamily: 'Inter', fontWeight: FontWeight.w500)),
                ],
              ),
            ),
            ...
            TextField(
              ...
              maxLines: 3,
              decoration: InputDecoration(
                hintText: 'Ex: FC máx 90 bpm, duração 15 min',
                hintStyle: const TextStyle(color: Color(0xFF9CA3AF)),
                filled: true,
                fillColor: const Color(0xFFF5F3EF),
                border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none),
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(color: const Color(0xFF4F4A34), borderRadius: BorderRadius.circular(12)),
                child: Center(
                  child: Text(isEditing ? 'Salvar Alterações' : 'Adicionar', style: const TextStyle(color: Colors.white, fontSize: 16, fontFamily: 'Inter', fontWeight: FontWeight.w600)),
                ),
              ),
            ),
            const SizedBox(height: 16),
