...
class _ClinicDocumentsScreenState extends State<ClinicDocumentsScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      context.read<ClinicContentProvider>().loadContentsByType('DOCUMENTS');
    });
  }
  ...
  void _navigateToPatientDetail(PatientListItem patient) {
    Navigator.push(context, MaterialPageRoute(builder: (context) => _PatientDocumentsDetailScreen(patient: patient)));
  }
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const Text('Documentos', style: TextStyle(color: Color(0xFF4F4A34), fontSize: 24, fontFamily: 'Inter', fontWeight: FontWeight.w500)),
        ],
      ),
    );
  }
  ...
  Widget _buildSelectPatientCard() {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Container(width: 64, height: 64, decoration: BoxDecoration(color: const Color(0xFFF5F3EF), borderRadius: BorderRadius.circular(32)), child: const Icon(Icons.folder_outlined, color: Color(0xFF697282), size: 32)),
          const SizedBox(height: 20),
          const Text('Selecione um paciente', style: TextStyle(color: Color(0xFF212621), fontSize: 16, fontFamily: 'Inter', fontWeight: FontWeight.w600)),
          const SizedBox(height: 8),
          const Text('para visualizar e gerenciar\nos documentos importantes', textAlign: TextAlign.center, style: TextStyle(color: Color(0xFF697282), fontSize: 14, fontFamily: 'Inter', fontWeight: FontWeight.w400)),
        ],
      ),
    );
  }
...
class _PatientDocumentsDetailScreenState extends State<_PatientDocumentsDetailScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (provider.currentType != 'DOCUMENTS') provider.loadContentsByType('DOCUMENTS');
    });
  }
  ...
  Widget _buildList(ClinicContentProvider provider) {
    ...
    if (items.isEmpty) {
      return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: const [Icon(Icons.folder_outlined, color: Color(0xFF697282), size: 48), SizedBox(height: 16), Text('Nenhum documento cadastrado', style: TextStyle(color: Color(0xFF697282), fontSize: 16, fontFamily: 'Inter')), SizedBox(height: 8), Text('Toque em "Adicionar Documento"', style: TextStyle(color: Color(0xFF697282), fontSize: 14, fontFamily: 'Inter'))]));
    }
    ...
    return Container(
      ...
      child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(isEditing ? 'Editar Documento' : 'Novo Documento', style: const TextStyle(color: Color(0xFF212621), fontSize: 20, fontFamily: 'Inter', fontWeight: FontWeight.w600)), GestureDetector(onTap: () => Navigator.pop(context), child: const Icon(Icons.close, color: Color(0xFF697282)))]),
          const SizedBox(height: 24),
          const Text('Título', style: TextStyle(color: Color(0xFF4F4A34), fontSize: 14, fontFamily: 'Inter', fontWeight: FontWeight.w500)),
          const SizedBox(height: 8),
          TextField(controller: _titleController, decoration: InputDecoration(hintText: 'Ex: Termo de Consentimento', hintStyle: const TextStyle(color: Color(0xFF9CA3AF)), filled: true, fillColor: const Color(0xFFF5F3EF), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none), contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14))),
          const SizedBox(height: 16),
          const Text('Descrição', style: TextStyle(color: Color(0xFF4F4A34), fontSize: 14, fontFamily: 'Inter', fontWeight: FontWeight.w500)),
          const SizedBox(height: 8),
          TextField(controller: _descriptionController, maxLines: 3, decoration: InputDecoration(hintText: 'Detalhes sobre o documento...', hintStyle: const TextStyle(color: Color(0xFF9CA3AF)), filled: true, fillColor: const Color(0xFFF5F3EF), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none), contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14))),
          const SizedBox(height: 24),
